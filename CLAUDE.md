...
❌ **Não inserir em `notifications` direto.** Notificações que cruzam usuários (técnico ↔ gestores) passam por `src/lib/notifications/notify.ts` (service role, `server-only`), que resolve `users.id` a partir de `technician_id` — são entidades diferentes, e confundi-las faz a notificação sumir sem erro. É esse mesmo módulo que dispara o **Web Push** (app fechado) logo após o insert — todo novo ponto de notificação usa `notify.ts` e ganha push de graça. Ver [ADR-017](./docs/architecture/ADR-017-notificacoes-realtime.md) e [ADR-018](./docs/architecture/ADR-018-push-app-fechado.md).
...
❌ **Não cachear HTML autenticado nem respostas do Supabase no service worker.** As telas do técnico são `force-dynamic` com dados por RLS (por usuário); servir uma página cacheada mostraria dados de outro técnico. `public/sw.js` usa network-first na navegação (com fallback `/offline.html`) e cache só de assets estáticos versionados. Ao mexer no SW, mantenha essa fronteira e suba `CACHE_VERSION` para ativar a versão nova. Ver [ADR-018](./docs/architecture/ADR-018-push-app-fechado.md) e o app do técnico (PWA/TWA).

❌ **Não setar `NEXT_PUBLIC_VAPID_PUBLIC_KEY` só em runtime.** Variáveis `NEXT_PUBLIC_*` são embutidas em **build time**; setar depois do deploy sem rebuildar deixa o valor `undefined` e a inscrição de push falha em silêncio. A chave privada VAPID é `server-only` (sem prefixo `NEXT_PUBLIC_`). O `assetlinks.json` (`public/.well-known/`) precisa ter `package_name` **idêntico** ao `packageId` do APK gerado, senão o Digital Asset Links não casa e a barra de URL não some.
