...
| ~~D~~ | ~~**Acesso do técnico ao portal**~~ | ✅ **Concluída (04/07)** — em `/equipe/tecnicos/[id]`: criar login (role tenant_technician), bloquear/reativar, resetar senha (`equipe/tecnicos/actions.ts`, `TechnicianAccess`) |
| ~~E~~ | ~~**Portal do técnico: pontos = payout**~~ | ✅ **Concluída (04/07)** — pontos do técnico agora somam o payout do período (helper único `(technician)/_lib/points.ts`, mesma base do "A pagar" do gestor); ranking anônimo via admin client. **Fix crítico** `service_visits.tecnico_id` (não `technician_id`) que zerava as telas — branch `fix/tecnico-tecnicoid` (aguardando merge) |
| **F** | **Portal do técnico: seletor de mês (PENDENTE)** | ⏳ **Próxima tarefa.** Home e Visitas devem ter seletor de mês **limitado aos meses com dados** (hoje mai/jun; jul vazio). Home: mês corrente vazio → nada, mas técnico filtra mai/jun p/ ver o **pendente de pagamento**. Visitas: `buildPeriodOptions` mostra range fixo → limitar aos meses existentes. Helper de meses distintos por `tecnico_id`; reusar `PeriodSelector`; pontos/mês via `_lib/points.ts` |
